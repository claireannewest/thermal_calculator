...
!         |-- SUBROUTINE INTEGRATION
 ...
 !--------------------------- v7.3.2 --------------------------------
   ...
   USE DDCOMMON_0
   IMPLICIT NONE
   INTEGER :: IERR,NRFLD                      ! Integer-MPI-ERRor; Near-Field
   CHARACTER(len=70) :: CMSGNM                ! Character-Message-Name
   CHARACTER(len=60) :: CFLPAR_DEFAULT        ! Character-File-Parameter-Default
   ! Change the dimension of COMMAND (next line) if # of input argument > 99.
   CHARACTER(len=60),dimension(999) :: COMMAND ! To store input parameter files
   integer :: nargin_par, it_argin            ! Number of input argument; iteration number for input argument
...
! initialize NRFLD to 0 before first call to DDSCAT
   nargin_par = iargc()
   if (nargin_par>=1) then
       do it_argin=1,nargin_par
           call getarg(it_argin,COMMAND(it_argin))
       end do
   else if (nargin_par==0) then
       COMMAND(1)='ddscat.par'
       nargin_par=1
   end if

   open(unit=876,file='Integration_f11f11',action='write',position='append',status='unknown')
   write(876,'(a10,3a10,2a20)') 'WAVE [um]','xc-gauss','yc-gauss','zc-gauss','Int(<|f11|^2>) [1]','C_sca [um^2]'
   close(unit=876)
 
   do it_argin = 1,nargin_par    
      CFLPAR = COMMAND(it_argin) 
      WRITE(0,FMT='(a,i3,a)')'>DDSCAT using parameter file #', it_argin,' =',trim(CFLPAR)
      ...
      ! Rename output files -- kong 
      if (nargin_par>1) then
         call execute_command_line('mv mtable mtable_'//CFLPAR) 
         call execute_command_line('mv qtable qtable_'//CFLPAR) 
         call execute_command_line('mv qtable2 qtable2_'//CFLPAR)
      end if
      call execute_command_line('rm temp_IXYZ0 target.out &
                  w???r???.avg w???r???k???.pol? w???r???k???.E*')
      
   end do ! for nargin_par
 END PROGRAM MAIN
...
! Local variables:
      ...
      REAL(WP) :: AEFF,AK1,AK3,BETAD,BETAMI,BETAMX,BETMID,BETMXD,COSBET,      &
		  ...
		  MB,NAMBIENT,PHID,PHIMAX,PHIMID,PHIMIN,PHIMXD,PI,PIA2,PYDDX, &
                  ...
                  YMIN,ZMAX,ZMIN                                              !
   ! Arguments for Gaussian beam incident
   integer :: flgwav            ! Option for wavefront. 0 -- planewave; 1 -- Gaussian
   real(WP),dimension(3)::xyzc0 ! Gaussian beam center; in unit of d (dipole spacing)
   real(WP)::w0                 ! Gaussian beam waist; in unit of d (dipole spacing)
   real(WP)::integ              ! Integration of |f11|^2 for power flow at selected directions.
! Target Properties:
...
!***
         ...
         CALL REAPAR(CBINFLAG,AEFFA,BETAMI,BETAMX,CALPHA,CFLEPS,CFLPAR,     &
                     ...
                     flgwav,xyzc0,w0) ! Arguments for gaussian beam
...
!***
            ...
            ! open(unit=76,file='temp_IXYZ0',action='write')
            ! do J = 1,nat0
            !    write(76,*) IXYZ0(J,1:3)
            ! end do
            ! close(76)
...
!***
                     ...
                     CALL NEARFIELD(CSTAMP,VERSNUM,NRWORD,CMDFFT,CFLE1,CFLE2, &
                                    CFLEB1,CFLEB2,MYID,AK1,AK_TF,IOCC,IXYZ0, &
...
!***
                     CALL GETFML(AK_TF,AK3,AKS_TF,BETADF,PHIDF,THETADF,DX,X0, &
                                 ...
                                 TOL,TIMERS,MXTIMERS,NTIMERS,NLAR,flgwav,xyzc0,w0,CFLAVG)
...
!**** If IWRKSC=1, write scattering properties of current orientation:
                     ...
                     IF(IWRKSC==1)THEN
                         ...
                         call integration(CFLAVG,integ)
                         open(unit=876,file='Integration_f11f11',action='write',position='append',status='unknown')
                         write(876,'(f10.4,3f10.2,2es20.8e3)')WAVE,xyzc0,integ,integ/(NAMBIENT*8._WP*atan(1._WP)/WAVE)**2
                         close(unit=876) 
                     ENDIF
!*** diagnostic
...
subroutine integration(filename,integ)
  use ddprecision,only:WP
  implicit none
  real(WP),parameter::PI=4._WP*atan(1.0_WP)
 
  character(len=12),intent(in):: filename ! ='w000r000.avg'
  real(WP),intent(out)::integ
  
  real(WP),dimension(:),allocatable::thetad0,phid0,data0
  real(WP),dimension(:),allocatable::theta0,phi0
  real(WP),dimension(:,:),allocatable::data00 ! data0*sin(theta)
  integer :: ND
  character(len=20):: line
  integer :: ss,s1,sr,sn,it,it2,nlines,Ltheta,Lphi
  
  ! Count number of lines in the file
  s1=0; nlines=0; sn=0
  open(33,file=filename,status='old',action='read',iostat=ss)
  if (ss==0) then
      do while (s1==0) 
          read(33,'(a)',iostat=s1) line
          nlines=nlines+1
          sr = index(line,'Selected scat')
          if (sr /= 0) then
             sn = nlines+1
          end if
      end do
  else
      write(*,*) 'Failed to read file, ',filename
      return
  end if
  close(33)
  nlines=nlines-1
  
  ! Load the 2nd-4th collums
  allocate(thetad0(nlines-sn),phid0(nlines-sn),data0(nlines-sn))
  open(33,file=filename,status='old',action='read')
  do it=1,sn
      read(33,'(a)') line
  end do
  do it=1,nlines-sn
      read(33,*) ND,thetad0(it),phid0(it),data0(it)
  end do
  close(33)
  
  ! Count numbers of angles theta and phi
  Ltheta=1;Lphi=0
  do it=2,nlines-sn
      if (thetad0(2)-thetad0(1)/=0) then
         Ltheta=nlines-sn
         if (thetad0(it)==thetad0(1)) then
             Ltheta=it-1
             exit
         end if
      end if
  end do
  Lphi=(nlines-sn)/Ltheta
   
  ! Built 2D array of data*sin(theta) with respect to phi and theta
  ! Differential solid angle = sin(theta) * dtheta * dphi
  allocate(theta0(Ltheta),phi0(Lphi),data00(Ltheta,Lphi))
  theta0=thetad0(1:Ltheta)*PI/180._WP
  it2=1
  do it=1,nlines-sn,Ltheta
      phi0(it2)=phid0(it)*PI/180._WP
      it2=it2+1
  end do
  do it=1,Lphi
      data00(1:Ltheta,it)=data0((it-1)*Ltheta+1:it*Ltheta)*sin(theta0)
  end do

  integ=0._WP 
  ! Return the surface integration only for a true 2D data array, i.e.,
  ! Ltheta,Lphi >=2
  if (Ltheta>=2 .and. Lphi>=2) then
      ! We should weight the edges (weight = 1/2) and corners (weight = 1/4)
      ! of the 2D data array, data00, for the integration. 
      
      data00(1,:)=data00(1,:)*0.5_WP
      data00(Ltheta,:)=data00(Ltheta,:)*0.5_WP
      data00(:,1)=data00(:,1)*0.5_WP
      data00(:,Lphi)=data00(:,Lphi)*0.5_WP
      
      ! Calculate the integration
      integ=sum(data00)*(theta0(2)-theta0(1))*(phi0(2)-phi0(1))
  else
      write(*,'(a)') '>> In integration.f90: integration is only returned &
      when directional angles of both theta and phi are multiple (length > 1).'
      write(*,'(2(a,i3),a)')'>>> But for the current data set, Ltheta =', &
                            Ltheta,'; Lphi =',Lphi,'.' 
  end if 
   
  deallocate(theta0,phi0,data00)
  deallocate(thetad0,phid0,data0)
